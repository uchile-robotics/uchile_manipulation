...
*/
#include "uchile_workspace_analysis/capability_map_plugin.h"
...
namespace move_group
{
    ...
    */
    if (node_handle_.hasParam("capability_map"))
    {
      ...
      for(XmlRpc::XmlRpcValue::ValueStruct::iterator it = raw_parameters.begin(); it != raw_parameters.end(); ++it)
      {
        ...
        uchile_workspace_analysis::CapabilityMapOptions opt;
        opt.load(nh_capmap);
        ...
        */
        if (node_handle_.hasParam("grasp_generator"))
        {
          ...
          uchile_grasp_generator::GraspOptions grasp_opt;
          grasp_opt.load(grasp_nh, opt.group_name);
          ros::NodeHandle grasp_gren_nh(grasp_nh, "cylindrical_grasp_generator");
          uchile_grasp_generator::CylindricalGraspGeneratorPtr grasp_gen(
              new uchile_grasp_generator::CylindricalGraspGenerator(grasp_gren_nh, grasp_opt));
          grasp_gen_.insert(std::make_pair<std::string, uchile_grasp_generator::CylindricalGraspGeneratorPtr>(opt.group_name, grasp_gen));
        }
        else
        {
        ...
        * Grasp filter
        */
        uchile_grasp_generator::GraspFilterPtr filter(new uchile_grasp_generator::GraspFilter(robot_state, opt.group_name));

        /* --------------------------------------------------------------------------------
        * Create tables
        */
        capmap_opt_.insert(std::make_pair<std::string, uchile_workspace_analysis::CapabilityMapOptions>(opt.group_name, opt));
        db_.insert(std::make_pair<std::string, GraspStorageDbPtr>(opt.group_name, db));
        grasp_filter_.insert(std::make_pair<std::string, uchile_grasp_generator::GraspFilterPtr>(opt.group_name, filter));
      }
    }
    else
    {
  ...
  bool CapabilityMapPlugin::getCapabilityMapCb(uchile_workspace_analysis::GetCapabilityMap::Request &req,
                                               uchile_workspace_analysis::GetCapabilityMap::Response &res)
  {
    // Check db connection
    ...
    uchile_workspace_analysis::CapabilityMapOptions& opt = capmap_opt_[req.group_name];

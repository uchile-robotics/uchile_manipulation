...
private:
  ...
  // Grasp generator
  uchile_grasp_generator::CylindricalGraspGeneratorPtr simple_grasps_;

  // Tool for visualizing things in Rviz
  moveit_visual_tools::MoveItVisualToolsPtr visual_tools_;

  // Grasp filter
  uchile_grasp_generator::GraspFilterPtr grasp_filter_;

  // data for generating grasps
  uchile_grasp_generator::CylindricalGraspGeneratorOptions grasp_data_;
...
public:
    ...
    : nh_("~")
  {
    ...
    simple_grasps_.reset(new uchile_grasp_generator::CylindricalGraspGenerator(grasp_nh, opt));


    // ---------------------------------------------------------------------------------------------
    // Load Grasp filter
    const robot_state::RobotState& robot_state = planning_scene_monitor_->getPlanningScene()->getCurrentState();
    grasp_filter_.reset(new uchile_grasp_generator::GraspFilter(robot_state, planning_group_name_) );
...
int main(int argc, char *argv[])
{
  ...
  // Run Tests
  uchile_grasp_generator::GraspGeneratorTest tester;

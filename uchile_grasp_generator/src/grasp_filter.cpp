                               ...
                               const double override_ik_timeout)
{
  ...
  ROS_INFO_STREAM_NAMED(name_, "Filtering " << possible_grasps.size() << " possible grasps with " << num_threads << " threads");
  ...
  for (int i = 0; i < num_threads; ++i)
  {
    ...
    IkThreadStruct tc(possible_grasps,
                      filtered_grasps,
                      ik_solutions,
                      link_transform,
                      grasps_id_start,
  ...
  ROS_INFO_STREAM_NAMED(name_,
                        "Grasp filter complete, found " << filtered_grasps.size() << " IK solutions out of "
  ...
  if (verbose_)
  {
    // End Benchmark time
    ROS_INFO_STREAM_NAMED(name_,
                          "Grasp generator IK grasp filtering benchmark time: "
  ...
  for (int i = ik_thread_struct.grasps_id_start_; i < ik_thread_struct.grasps_id_end_; ++i)
  {
    ...
    if (error_code.val == moveit_msgs::MoveItErrorCodes::SUCCESS)
    {
      ...
      if (ik_thread_struct.filter_pregrasp_)       // optionally check the pregrasp
      {
        ...
        ik_pose =
            uchile_grasp_generator::getPreGraspPose(ik_thread_struct.possible_grasps_[i], ik_thread_struct.ee_parent_link_);
        ...
        {
          ROS_DEBUG_NAMED(name_, "Unable to find IK solution for pre-grasp pose.");
          continue;
        }
        else if (error_code.val == moveit_msgs::MoveItErrorCodes::TIMED_OUT)
        {
          ROS_DEBUG_NAMED(name_, "Unable to find IK solution for pre-grasp pose: Timed Out.");
          continue;
        }
        ...
        {
          ROS_INFO_STREAM_NAMED(name_, "IK solution error for pre-grasp: MoveItErrorCodes.msg = " << error_code);
        }
      }
